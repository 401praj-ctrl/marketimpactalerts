          ...
          child: ListTile(
            onTap: () {
              final alert = EventAlert(
                ...
                impactDescription: pred['impact_description'] ?? 'Historical prediction record from the accuracy dashboard.',
                eventDate: pred['timestamp']?.toString().split('T')[0] ?? '',
                impactDateEst: pred['impact_date_est'] ?? '',
                probability: ((pred['probability'] ?? 0.5) * 100).toInt(),
                reason: pred['reason'] ?? 'Prediction stored on ${pred['timestamp']}',
                impactType: pred['impact_type'] ?? 'Direct',
